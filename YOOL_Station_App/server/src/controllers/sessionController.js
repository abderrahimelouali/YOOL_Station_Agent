...
exports.verifyProxy = async (req, res) => {
    const { station_code, station_key, card_identifier, workspace_id } = req.body;

    // Validation des entrées obligatoires
    if (!station_code || !station_key || !card_identifier) {
    ...
    try {
        ...
        let stations = await db.query('SELECT station_key_hash, status FROM stations WHERE station_code = ?', [station_code]);

        // Si la station est nouvelle, on l'inscrit automatiquement en mode 'pending'
        if (stations.length === 0) {
            const hash = await bcrypt.hash(station_key, 10);
            ...
            await db.query(
                'INSERT INTO stations (station_code, station_key_hash, status, location) VALUES (?, ?, ?, ?)',
        ...
        const isMatch = await bcrypt.compare(station_key, station.station_key_hash);
        if (!isMatch) {
        ...
        const response = await axios.post(`${cardSystemUrl}/cards/verify`, {
            card_identifier,
            workspace_id,
            station_key: masterSecret // <--- CORRECT
        });
...
exports.startSession = async (req, res) => {
    const { station_code, station_key, card_uid, student_id, student_name } = req.body;

    if (!station_code || !station_key || !card_uid || !student_id) {
        return res.status(400).json({ success: false, message: 'Données incomplètes' });
    }

    try {
        // Authentification de la station
        const stations = await db.query('SELECT id, station_key_hash, status FROM stations WHERE station_code = ?', [station_code]);
        if (stations.length === 0 || !(await bcrypt.compare(station_key, stations[0].station_key_hash))) {
...
exports.heartbeat = async (req, res) => {
    const { station_code, station_key } = req.body;

    if (!station_code || !station_key) return res.status(400).json({ success: false });

    try {
        const stations = await db.query('SELECT station_key_hash FROM stations WHERE station_code = ?', [station_code]);
        if (stations.length > 0 && await bcrypt.compare(station_key, stations[0].station_key_hash)) {
            await db.query('UPDATE stations SET last_seen = NOW() WHERE station_code = ?', [station_code]);
